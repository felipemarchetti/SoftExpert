{
	...
	class Details {
		...
		initEvents() {
			function isBootstrapModalOpen() {
				return document.querySelector('.modal-backdrop.fade.show') !== null;
			}
		
			function isSweetAlertModalOpen() {
				return document.querySelector('.swal2-container') !== null;
			}
		
			// close page when outside of page is clicked, only if modals are not open
			document.body.addEventListener('click', () => {
				if (!isBootstrapModalOpen() && !isSweetAlertModalOpen()) {
					this.close();
				}
			});
	...
	document.addEventListener("DOMContentLoaded", function() {
		var modal = document.getElementById('gridProdutos');
		var modalOut = document.getElementById('mainContent');
		if (modal) {
			modal.click();

			var detailsDiv = document.querySelector('.details--open');
		...
		$('#modalExibeVendas, #pdvModal').on('show.bs.modal', function () {
			closeModal();
		});
	});
